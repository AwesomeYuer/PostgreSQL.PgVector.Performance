...
public class ChromaVectorDbClient
{
    ...
    {
        this._chromaLog.LogDebug("Searching vectors by point ID");
        ...
        {
            this._chromaLog.LogDebug("Vectors not found {0}", e.Message);
            yield break;
        }
        ...
        if (data == null)
        {
            this._chromaLog.LogWarning("Unable to deserialize Get response");
            yield break;
        }

        if (!data.Result.Any())
        {
            this._chromaLog.LogWarning("Vectors not found");
            yield break;
        }
...
#pragma warning restore CS8604
    ...
    public async Task<ChromaVectorRecord?> GetVectorByPayloadIdAsync(string collectionName, string metadataId, bool withVector = false, CancellationToken cancel = default)
    {
        ...
        {
            this._chromaLog.LogDebug("Request for vector with payload ID failed {0}", e.Message);
            return null;
        }
        ...
        if (data == null)
        {
            this._chromaLog.LogWarning("Unable to deserialize Search response");
            return null;
        }

        if (!data.Results.Any())
        {
            this._chromaLog.LogDebug("Vector not found");
            return null;
        }
        ...
        var record = new ChromaVectorRecord(
            ...
            tags: null);
        this._chromaLog.LogDebug("Vector found}");
    ...
    {
        this._chromaLog.LogDebug("Deleting vector by point ID");
        ...
        try
        {
            ...
            if (result?.Status == "ok")
            {
                this._chromaLog.LogDebug("Vector being deleted");
            }
            else
            {
                this._chromaLog.LogWarning("Vector delete failed");
            }
        }
        catch (HttpRequestException e)
        {
            this._chromaLog.LogError(e, "Vector delete request failed: {0}", e.Message);
        }
    }
    ...
    public async Task DeleteVectorByPayloadIdAsync(string collectionName, string metadataId, CancellationToken cancel = default)
    {
        ...
        {
            this._chromaLog.LogDebug("Vector not found, nothing to delete");
            return;
        }

        this._chromaLog.LogDebug("Vector found, deleting");
    ...
    {
        this._chromaLog.LogDebug("Upserting vectors");
        ...
        try
        {
            ...
            if (result?.Status == "ok")
            {
                this._chromaLog.LogDebug("Vectors upserted");
            }
            else
            {
                this._chromaLog.LogWarning("Vector upserts failed");
            }
        }
        catch (HttpRequestException e)
        {
            this._chromaLog.LogError(e, "Vector upserts request failed: {0}", e.Message);
        }
    }
    ...
    public async IAsyncEnumerable<(ChromaVectorRecord, double)> FindNearestInCollectionAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancel = default)
    {
        this._chromaLog.LogDebug("Searching top {0} nearest vectors", top);
        ...
        if (!data.Results.Any())
        {
            this._chromaLog.LogWarning("Nothing found");
            yield break;
        }
    ...
    public async Task CreateCollectionAsync(string collectionName, CancellationToken cancel = default)
    {
        ...
        this._chromaLog.LogDebug("Creating collection {0}", collectionName);
    ...
    {
        ChromaMemoryStore chromaMemStore;
        this._chromaLog.LogDebug("Fetching collection {0}", collectionName);
       ...
       using var request = GetCollectionsRequest.Create(collectionName).Build();
        ...
        {
            this._chromaLog.LogError("Collection fetch failed: {0}, {1}", response.StatusCode, responseContent);
            return false;
        }
    }
    ...
    {
        this._chromaLog.LogDebug("Listing collections");
    ...
    {
        HttpResponseMessage response = await this._httpChromaClient.SendAsync(request, cancel).ConfigureAwait(false);

        string responseContent = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
        if (response.IsSuccessStatusCode)
        {
            this._chromaLog.LogTrace("Chroma responded successfully");
        }
        else
        {
            this._chromaLog.LogTrace("Chroma responded with error");
        }
